...
(function () {
  ...
  {
    id: 9,
    question: "국기원 정식 승품·승단 심사를 거쳐 공식 품증 또는 단증을 발급받은 적이 있나요?",
    options: [
      { text: '아직 국기원 심사를 보지 않았거나, 승품·승단 심사를 목표로 열심히 수련 중이다.', target: 'RED_BELT' },
      { text: '국기원 1품~4품 심사에 합격하여 품증(품띠/검은띠)을 당당히 보유하고 있다.', target: 'POOM_1' },
      { text: '국기원 1단~5단 심사에 합격하여 정식 검은 띠 단증을 보유하고 있다.', target: 'DAN_1' },
      { text: '국기원 4단 이상 고단자 또는 지도자 자격을 갖추고 수련생을 지도하고 있다.', target: 'SABEOM' }
    ]
  },
  {
    id: 10,
  ...
  class CardExporter {
    ...
    static exportInstaStoryPNG(resultData) {
      ...
      a.href = dataUrl;
      const namePart = resultData.userName ? `_${resultData.userName}` : '';
      a.download = `인스타그램_스토리_인증서${namePart}_${resultData.type.replace(/\s+/g, '_')}.png`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    }
    ...
    render() {
      ...
      targetNode.innerHTML = `
        <div class="glass-card landing-view">
          ...
          <p style="font-size: 0.75rem; color: #64748B; margin-top: 14px; text-align: center; line-height: 1.5;">
            💡 본 테스트는 재미와 즐거운 수련 동기부여를 위해 제작된 콘텐츠입니다.<br>
            🔒 입력한 이름과 도장 정보는 서버에 수집·저장되지 않으며 일시 사용됩니다.
          </p>
        </div>
      `;
  ...
  class ResultScreenComponent {
    ...
    shareInstagramStory(resultData) {
      ...
      if (isIOS || isAndroid) {
        alert(
          '📸 9:16 인스타그램 스토리 인증서 다운로드 & 링크 복사 완료!\n\n' +
          '📌 [스토리 공유 방법]\n' +
          '1. 아래 [확인]을 누르면 인스타그램 스토리 카메라로 이동합니다.\n' +
          '2. 갤러리에서 저장된 [9:16 인증서 이미지]를 선택하세요.\n' +
          '3. 상단 메뉴의 [스티커 버튼 😃] ➔ [🔗 링크] 스티커를 터치하세요.\n' +
          '4. 복사된 테스트 URL 주소를 붙여넣고 스토리에 올려보세요!'
        );
